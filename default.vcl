...
backend server1 { # Define one backend
  .host = "x.x.x.x";    # IP or Hostname of backend
  .port = "6081";           # Port Apache or whatever is listening
...
sub vcl_recv {
  ...
  #Should detect the device first
  call detect_device;
  ...
  # Should update the source ip to be x-forwarded
  ...
  if (req.url == "/promotion" || req.url == "/" || req.url == "/casino" || req.url == "/sportsbook" || req.url == "/idnpoker" || req.url == "/slots" || req.url == "/idnlive" || req.url == "/referrals" || req.url == "/undefined") {
   if (req.method == "POST") {
    return (synth(404, "Not found"));
   }
  }

  if (req.url ~ "/assets/themes/idns/" || req.url ~ "/dashhome") {
    return (synth(404, "Not found"));
  }
